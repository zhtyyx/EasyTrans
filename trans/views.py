...
# 百度翻译调用 方法接口在 translate_func.py文件中
@csrf_exempt
def baidu_trans(request):
    if request.method == "GET":
        return render(request, "index.html")
    else:
        content = request.POST.get("content")
        baidu_trans_content = translate_func.baidu_translate(content)
        return HttpResponse(baidu_trans_content)
...
def upload_func(request):
    if request.method == "GET":
        ...
        if str(file_obj.name).endswith('.pdf') or str(file_obj.name).endswith('.doc') or str(file_obj.name).endswith(
                '.docx'):
...
def upload_trans(request):
    ...
    if str(file_name).endswith('.pdf'):  # 如果后缀为.pdf则执行打开pdf文件的操作
        try:
            ...
            if round(fsize/float(1024*1024), 2) > 15:
                return HttpResponse("oversize")
            res = trans_to_pdf.trans_pdf(file_name, path)  # 调用pdf翻译类的函数
